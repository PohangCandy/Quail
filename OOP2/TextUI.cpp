...
TextUI::TextUI(GameObject* parent)
	: GameObject("", 1), parent(parent), hp(0.0f), n_remaining_frames(60), damageable(dynamic_cast<IDamageable*>(parent))
{
	if (parent == nullptr)
		return;
}

void TextUI::update(const Canvas* canvas) {
	char buf[10];
...
void TextUI::draw(Canvas* canvas) const {
